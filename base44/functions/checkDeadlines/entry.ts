    ...
    for (const task of tasks) {
      ...
      if ([30, 14, 7].includes(daysUntilDue) && client?.contact_email) {
        ...
        await base44.asServiceRole.integrations.Core.SendEmail({
          ...
          subject: `[${urgency}] ${task.title} — Due in ${daysUntilDue} days`,
          body: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
              ...
              <div style="padding: 24px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 8px 8px;">
                <p>Dear ${client.contact_name || 'Plan Administrator'},</p>
                ...
                <div style="background: #f8fafc; padding: 16px; border-radius: 8px; margin: 16px 0; border-left: 4px solid ${daysUntilDue <= 7 ? '#ef4444' : '#f59e0b'};">
                  ...
                  ${task.description ? `<p style="margin: 8px 0 0; color: #6b7280; font-size: 14px;">${task.description}</p>` : ''}
                </div>
